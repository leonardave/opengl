 ...
 *
 */
bool ShaderLoader::initFromString(const char* vertShader, const char* fragShader)
{
    //临时着色器对象
    GLuint vsId, fsId;
    compileShader(&vsId, GL_VERTEX_SHADER, vertShader);
    compileShader(&fsId, GL_FRAGMENT_SHADER, fragShader);
...
bool ShaderLoader::compileShader(GLuint* shader, GLenum shaderType, const GLchar* source)
{
    //
    ...
    glShaderSource(shaderId,              //GLuint shader,指定要替换源代码的Shader
                   1,                     //GLsizei count,字符指针数组string中的元素个数
                   (const char**)&source, //const GLchar **string,字符指针数组
    ...
    if (succ == GL_FALSE)
    {
        ...
        fprintf(stderr, "Unable to compile shader %s: log is%s\n", source, log);
        free(log);
